...
const filename = "generated_chains_avalanche.go"
...
var (
{{ range . }}
	{{.VarName}} = AvalancheChain{ChainID: "{{ .ChainID }}", Selector: {{ .Selector }}, Name: "{{ .Name }}"}{{ end }}
)
...
func main() {
	...
	if string(existingContent) == string(formatted) {
		fmt.Println("avalanche: no changes detected")
		return
	}
	fmt.Println("avalanche: updating generations")
...
func genChainsSourceCode() (string, error) {
	...
	for ChainID, chainSel := range chain_selectors.AvalancheChainIdToChainSelector() {
		name, err := chain_selectors.AvalancheNameFromChainId(ChainID)
		if err != nil {
			return "", err
		}
