...
const filename = "generated_chains_dogecoin.go"
...
var (
{{ range . }}
	{{.VarName}} = DogecoinChain{ChainID: "{{ .ChainID }}", Selector: {{ .Selector }}, Name: "{{ .Name }}"}{{ end }}
)
...
func main() {
	...
	if string(existingContent) == string(formatted) {
		fmt.Println("dogecoin: no changes detected")
		return
	}
	fmt.Println("dogecoin: updating generations")
...
func genChainsSourceCode() (string, error) {
	...
	for ChainID, chainSel := range chain_selectors.DogecoinChainIdToChainSelector() {
		name, err := chain_selectors.DogecoinNameFromChainId(ChainID)
		if err != nil {
			return "", err
		}
