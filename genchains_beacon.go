...
const filename = "generated_chains_beacon.go"
...
var (
{{ range . }}
	{{.VarName}} = BeaconChain{ChainID: {{ .ChainID }}, Selector: {{ .Selector }}, Name: "{{ .Name }}"}{{ end }}
)
...
func main() {
	...
	if string(existingContent) == string(formatted) {
		fmt.Println("beacon: no changes detected")
		return
	}
	fmt.Println("beacon: updating generations")
...
func genChainsSourceCode() (string, error) {
	...
	for ChainID, chainSel := range chain_selectors.BeaconChainIdToChainSelector() {
		name, err := chain_selectors.BeaconNameFromChainId(ChainID)
		if err != nil {
			return "", err
		}
